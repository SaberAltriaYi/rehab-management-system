...
9. 显示租户“工作室内部”、账号 `admin` 和 16 位随机初始管理员密码，并允许从系统浏览器打开
   HTTPS 管理端。

请先保存初始密码，登录后立即修改，再点击“我已保存，停止显示”。初始密码、数据库密码、
Redis 密码和 Token 不进入启动器日志或诊断信息。

`desktop-v1.0.0-preview.1` 曾错误复用 48 位基础设施密钥生成器来创建管理员临时密码。当前
构建将管理员密码与基础设施密钥分开生成，并为旧包保留最长 64 位的登录及旧密码兼容入口；
新密码、重置密码和后续新安装仍限制为 4–16 位。
...
- 健康后打开系统；
- 创建数据库与附件的 age 加密备份；
- 进入后台管理，并在启动器中安全更新内置超级管理员账号和 12–16 位密码；
- 一键导出 age 加密整店迁移包，包含数据库、系统用户/权限与附件；
- 一键导入整店迁移包，导入前自动创建目标设备备份，再全量覆盖数据库与附件；
...
整店迁移包不包含目标设备的 MySQL/Redis 运行密码、TLS 私钥、端口、日志、缓存或本机备份。
导入会保留这些机器级配置。迁移包仍包含患者数据，必须使用至少 12 位密码，并将文件与密码
通过不同的受控渠道传递。覆盖导入需要输入 `覆盖导入全部数据` 并再次确认。
