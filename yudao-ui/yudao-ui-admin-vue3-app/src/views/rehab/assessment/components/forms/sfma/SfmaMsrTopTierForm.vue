  ...
  <el-card shadow="never" class="mb-12px">
    <template #header>
      <div class="flex items-center justify-between">
        <span class="font-bold">多节段旋转 Top Tier（左/右精简初筛）</span>
    ...
    <el-alert
      ...
      type="info"
      class="mb-10px"
      title="Top Tier 仅保留分类、疼痛、简要备注与分解建议；详细观察请在 MSR Breakout 中记录。"
    />

    <el-row :gutter="12">
      <el-col :span="12">
        <el-card shadow="never">
          <template #header>多节段旋转（左）</template>
      ...
      <el-col :span="12">
        <el-card shadow="never">
          <template #header>多节段旋转（右）</template>
...
const buildDefaultByCode = (
  code: 'multi_segmental_rotation_left' | 'multi_segmental_rotation_right'
): SfmaTopTierRecord => {
...
const applyRulesForCode = (
  ...
  const actionZh = code === 'multi_segmental_rotation_left' ? '多节段旋转（左）' : '多节段旋转（右）'
  ...
  if (row.classification === 'FP') {
    return {
      ...
      breakout_reason_text: `${actionZh}为疼痛性功能模式，建议谨慎进入 MSR 分解评估。`,
  ...
  if (row.classification === 'DN') {
    return {
      ...
      needs_breakout_suggestion: true,
      breakout_reason_text: `${actionZh}存在功能异常，建议进入 MSR 分解评估。`,
  ...
  if (row.classification === 'DP') {
    return {
      ...
      pain_present: normalizePainPresent((rowInput as any)?.pain_present, true),
      needs_breakout_suggestion: true,
      breakout_reason_text: `${actionZh}存在功能异常并伴疼痛，建议优先人工复核并谨慎进入 MSR 分解评估。`,
      review_priority: 'high',
      caution_text: '优先疼痛管理/谨慎继续分解'
    }
  }
...
const applyRules = (
  ...
  left: applyRulesForCode(input?.left, 'multi_segmental_rotation_left'),
  right: applyRulesForCode(input?.right, 'multi_segmental_rotation_right')
})
