  ...
  <el-card shadow="never" class="mb-12px">
    <template #header>
      <div class="flex items-center justify-between">
        <span class="font-bold">上肢模式1 Top Tier（左/右精简初筛）</span>
    ...
    <el-alert
      ...
      type="info"
      class="mb-10px"
      title="Top Tier 仅保留分类、疼痛、简要备注与分解建议；详细观察请在上肢模式1 Breakout 中记录。"
    />

    <el-row :gutter="12">
      <el-col :span="12">
        <el-card shadow="never">
          <template #header>上肢模式1（左）</template>
          <el-alert
      ...
      <el-col :span="12">
        <el-card shadow="never">
          <template #header>上肢模式1（右）</template>
          <el-alert
...
type UeSide = 'left' | 'right'

const props = defineProps<{
  modelValue?: SfmaUpperExtremityPattern1TopTier
}>()

const emit = defineEmits<{
  (e: 'update:modelValue', value: SfmaUpperExtremityPattern1TopTier): void
  (e: 'change', value: SfmaUpperExtremityPattern1TopTier): void
  (e: 'breakout-action', value: { side: UeSide; action: 'enter' | 'skip' }): void
}>()

const deepClone = (value: any) => JSON.parse(JSON.stringify(value || {}))
const model = reactive<SfmaUpperExtremityPattern1TopTier>(buildDefaultUpperExtremityPattern1TopTier())
const syncFromProps = (value?: SfmaUpperExtremityPattern1TopTier) => {
  const payload = applyUpperExtremityPattern1TopTierRules(value || buildDefaultUpperExtremityPattern1TopTier())
  model.left = payload.left
  model.right = payload.right
}
const emitModel = () => {
  const payload = applyUpperExtremityPattern1TopTierRules(deepClone(model))
  model.left = payload.left
  model.right = payload.right
...
const sidePainConflict = (side: UeSide) => {
  const row = model[side]
  if (!row.classification) return false
  if ((row.classification === 'FP' || row.classification === 'DP') && !row.pain_present) return true
  if ((row.classification === 'FN' || row.classification === 'DN') && row.pain_present) return true
  return false
}
...
const handleClassificationChange = (side: UeSide) => {
  ...
  const payload = applyUpperExtremityPattern1TopTierRules(next)
  model.left = payload.left
  model.right = payload.right
  emitModel()
}

const emitAction = (side: UeSide, action: 'enter' | 'skip') => {
...
const reset = () => {
  const payload = buildDefaultUpperExtremityPattern1TopTier()
  model.left = payload.left
  model.right = payload.right
  emitModel()
}
