  ...
  <el-card shadow="never" class="mb-12px">
    <template #header>
      <div class="flex items-center justify-between">
        <span class="font-bold">多节段屈曲 Top Tier（精简初筛）</span>
    ...
    <el-alert
      ...
      type="info"
      class="mb-10px"
      title="Top Tier 仅保留分类、疼痛、简要备注与分解建议；详细观察请在 MSF Breakout 中记录。"
    />
      ...
      <el-row :gutter="12">
        ...
        <el-col :span="14">
          <el-form-item label="Breakout 目标">
            <el-input :model-value="model.needs_breakout_suggestion ? '多节段屈曲分解评估（msf_breakout）' : ''" disabled />
          </el-form-item>
        </el-col>
      </el-row>
...
const TEST_CODE = 'multi_segmental_flexion'
const TEST_NAME = '多节段屈曲（MSF）'
...
const applyRules = (input?: Partial<SfmaTopTierRecord>): SfmaTopTierRecord => {
  ...
  if (row.classification === 'FP') {
    return {
      ...
      breakout_reason_text: '多节段屈曲为疼痛性功能模式，建议谨慎进入 MSF 分解评估。',
  ...
  if (row.classification === 'DN') {
    return {
      ...
      needs_breakout_suggestion: true,
      breakout_reason_text: '多节段屈曲存在功能异常，建议进入 MSF 分解评估。',
  ...
  if (row.classification === 'DP') {
    return {
      ...
      pain_present: normalizePainPresent((input as any)?.pain_present, true),
      needs_breakout_suggestion: true,
      breakout_reason_text: '多节段屈曲存在功能异常并伴疼痛，建议优先人工复核并谨慎进入 MSF 分解评估。',
      review_priority: 'high',
      caution_text: '优先疼痛管理/谨慎继续分解'
    }
  }
