  ...
  <el-card shadow="never" class="mb-12px">
    <template #header>
      <div class="flex items-center justify-between">
        <span class="font-bold">颈椎屈曲 Breakout（高级评估）</span>
    ...
    <el-alert
      ...
      type="info"
      class="mb-10px"
      title="按教材流程记录：仰卧位主动屈曲 → 仰卧位被动屈曲 → 仰卧位寰枕关节主动屈曲（20°）。"
    />
    ...
    <el-form :model="model" label-width="168px">
      ...
      <el-divider content-position="left">步骤1：仰卧位主动屈曲</el-divider>
      <el-row :gutter="12">
        <el-col :span="8">
          <el-form-item label="主动屈曲质量">
            <el-radio-group v-model="model.active_cervical_flexion_quality" @change="emitChange">
        ...
        <el-col :span="5">
          <el-form-item label="主动疼痛">
            <el-switch v-model="model.active_cervical_flexion_pain" @change="handlePainAutoReview" />
          </el-form-item>
        </el-col>
        <el-col :span="5">
          <el-form-item label="主动ROM(°)">
            <el-input-number
              v-model="model.active_cervical_flexion_rom_key"
              :min="0"
              :max="180"
              class="!w-full"
              @change="emitChange"
        ...
        <el-col :span="6">
          <el-form-item label="终末感备注">
            <el-input v-model="model.active_cervical_flexion_end_feel_note" @input="emitChange" />
          </el-form-item>
        </el-col>
      </el-row>

      <el-divider content-position="left">步骤2：仰卧位被动屈曲</el-divider>
      <el-row :gutter="12">
        <el-col :span="8">
          <el-form-item label="被动屈曲质量">
            <el-radio-group v-model="model.passive_cervical_flexion_quality" @change="emitChange">
              <el-radio label="正常">正常</el-radio>
              <el-radio label="受限">受限</el-radio>
              <el-radio label="明显受限">明显受限</el-radio>
        ...
        <el-col :span="5">
          <el-form-item label="被动疼痛">
            <el-switch v-model="model.passive_cervical_flexion_pain" @change="handlePainAutoReview" />
          </el-form-item>
        </el-col>
        <el-col :span="5">
          <el-form-item label="被动ROM(°)">
            <el-input-number
              v-model="model.passive_cervical_flexion_rom_key"
              :min="0"
              :max="180"
              class="!w-full"
              @change="emitChange"
      ...
      <el-divider content-position="left">步骤3：仰卧位寰枕关节主动屈曲（20°）</el-divider>
      <el-row :gutter="12">
        <el-col :span="8">
          <el-form-item label="上位颈观察">
            <el-radio-group v-model="model.upper_cervical_flexion_observation" @change="emitChange">
              <el-radio label="正常">正常</el-radio>
      ...
      <el-form-item label="代偿模式">
        <el-checkbox-group v-model="model.compensation_patterns" @change="emitChange">
          <el-checkbox label="胸椎代偿屈曲" />
          <el-checkbox label="肩胛上提" />
          <el-checkbox label="肩部前伸" />
          <el-checkbox label="躯干前移" />
...
const props = defineProps<{
  modelValue?: SfmaCervicalFlexionBreakout
}>()

const emit = defineEmits<{
  (e: 'update:modelValue', value: SfmaCervicalFlexionBreakout): void
  (e: 'change', value: SfmaCervicalFlexionBreakout): void
}>()

const deepClone = (value: any) => JSON.parse(JSON.stringify(value || {}))

const model = computed<SfmaCervicalFlexionBreakout>({
  get: () => ({
    ...buildDefaultCervicalFlexionBreakout(),
    ...(props.modelValue || {})
  }),
...
const handlePainAutoReview = () => {
  ...
  if (
    next.active_cervical_flexion_pain ||
    next.passive_cervical_flexion_pain
  ) {
...
const statusLabel = (status: SfmaCervicalFlexionBreakout['breakout_status']) => {
...
const statusTagType = (status: SfmaCervicalFlexionBreakout['breakout_status']) => {
  if (status === 'completed') {
    return 'success'
  }
  if (status === 'in_progress' || status === 'partial') {
...
const reset = () => {
  model.value = buildDefaultCervicalFlexionBreakout()
}
