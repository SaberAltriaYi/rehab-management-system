  ...
  <el-card shadow="never" class="mb-12px">
    <template #header>
      <div class="flex items-center justify-between">
        <span class="font-bold">颈椎屈曲 Top Tier（精简初筛）</span>
...
const props = defineProps<{
  modelValue?: SfmaCervicalFlexionTopTier
}>()

const emit = defineEmits<{
  (e: 'update:modelValue', value: SfmaCervicalFlexionTopTier): void
  (e: 'change', value: SfmaCervicalFlexionTopTier): void
  (e: 'breakout-action', value: 'enter' | 'skip'): void
}>()

const deepClone = (value: any) => JSON.parse(JSON.stringify(value || {}))
const model = reactive<SfmaCervicalFlexionTopTier>(buildDefaultCervicalFlexionTopTier())
const syncFromProps = (value?: SfmaCervicalFlexionTopTier) => {
  Object.assign(model, applyCervicalFlexionTopTierRules(value || buildDefaultCervicalFlexionTopTier()))
}
const emitModel = () => {
  const payload = applyCervicalFlexionTopTierRules(deepClone(model))
...
const handleClassificationChange = () => {
  ...
  Object.assign(model, applyCervicalFlexionTopTierRules(next))
  emitModel()
}
...
const reset = () => {
  Object.assign(model, buildDefaultCervicalFlexionTopTier())
  emitModel()
}
