  ...
  <el-card shadow="never" class="mb-12px">
    <template #header>
      <div class="flex items-center justify-between">
        <span class="font-bold">上肢模式1 Breakout（左/右分解）</span>
    ...
    <el-row :gutter="12">
      <el-col v-for="side in sides" :key="side" :span="12">
        <el-card shadow="never">
          <template #header>上肢模式1 Breakout（{{ sideText(side) }}）</template>
          <el-form :model="model[side]" label-width="154px">
            ...
            <el-form-item label="主动测试结果">
              <el-radio-group v-model="model[side].prone_active_result" @change="emitChange(side)">
                <el-radio v-for="item in classOptions" :key="`ue1-active-${side}-${item}`" :label="item">{{ item }}</el-radio>
              </el-radio-group>
            </el-form-item>
            ...
            <template v-else>
              <el-form-item label="被动测试结果">
                <el-radio-group v-model="model[side].prone_passive_result" @change="emitChange(side)">
                  <el-radio v-for="item in classOptions" :key="`ue1-passive-${side}-${item}`" :label="item">{{ item }}</el-radio>
                </el-radio-group>
              </el-form-item>
            ...
            <template v-else>
              <el-form-item label="交互测试结果">
                <el-radio-group v-model="model[side].supine_interactive_result" @change="emitChange(side)">
                  <el-radio v-for="item in classOptions" :key="`ue1-supine-${side}-${item}`" :label="item">{{ item }}</el-radio>
                </el-radio-group>
              </el-form-item>
    ...
    <el-form :model="model" label-width="138px">
      <el-form-item label="左右差重点">
        <el-input
          ...
          type="textarea"
          :rows="2"
          placeholder="例如：左侧模式1在被动与交互阶段均明显差于右侧。"
          @input="emitChange()"
        />
      </el-form-item>
...
import {
  ...
  SfmaUpperExtremityPattern1BreakoutSide
...
const props = defineProps<{
  modelValue?: SfmaUpperExtremityPattern1Breakout
}>()

const emit = defineEmits<{
  (e: 'update:modelValue', value: SfmaUpperExtremityPattern1Breakout): void
  (e: 'change', value: SfmaUpperExtremityPattern1Breakout): void
}>()

const deepClone = (value: any) => JSON.parse(JSON.stringify(value || {}))

const mergeWithDefaults = (value?: SfmaUpperExtremityPattern1Breakout): SfmaUpperExtremityPattern1Breakout => {
  const defaults = buildDefaultUpperExtremityPattern1Breakout()
  const input = value || ({} as SfmaUpperExtremityPattern1Breakout)
  return {
    ...defaults,
    ...input,
    left: {
...
const model = computed<SfmaUpperExtremityPattern1Breakout>({
...
const needsPassiveTest = (sideData: SfmaUpperExtremityPattern1BreakoutSide) => {
  return isAbnormalClass(sideData.prone_active_result)
}

const needsSupineTest = (sideData: SfmaUpperExtremityPattern1BreakoutSide) => {
...
const flowHint = (sideData: SfmaUpperExtremityPattern1BreakoutSide) => {
  if (!sideData.prone_active_result) return '请先完成“俯卧位上肢主动动作模式”结果判定。'
  if (sideData.prone_active_result === 'FN') {
    return '主动测试为 FN：提示更偏姿势性和/或肩带稳定-运动控制问题，可进入控制训练分析。'
  }
...
const syncSideDerived = (sideData: SfmaUpperExtremityPattern1BreakoutSide) => {
  sideData.active_ue_pattern1_quality = mapActiveQuality(sideData.prone_active_result)
  sideData.active_ue_pattern1_pain = isPainClass(sideData.prone_active_result)
  ...
  const hasPain = sideData.active_ue_pattern1_pain || sideData.stop_and_treat
  if (hasPain) {
...
const emitChange = (side?: SideKey) => {
  const next = deepClone(model.value) as SfmaUpperExtremityPattern1Breakout
  if (side) {
...
const reset = () => {
  model.value = buildDefaultUpperExtremityPattern1Breakout()
}
