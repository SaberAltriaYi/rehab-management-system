  ...
  <el-card shadow="never" class="mb-12px">
    <template #header>
      <div class="flex items-center justify-between">
        <span class="font-bold">上肢模式2 Top Tier（左/右精简初筛）</span>
    ...
    <el-alert
      ...
      type="info"
      class="mb-10px"
      title="Top Tier 仅保留分类、疼痛、简要备注与分解建议；详细观察请在上肢模式2 Breakout 中记录。"
    />

    <el-row :gutter="12">
      <el-col :span="12">
        <el-card shadow="never">
          <template #header>上肢模式2（左）</template>
          <el-alert
      ...
      <el-col :span="12">
        <el-card shadow="never">
          <template #header>上肢模式2（右）</template>
          <el-alert
...
const props = defineProps<{
  modelValue?: SfmaUpperExtremityPattern2TopTier
}>()

const emit = defineEmits<{
  (e: 'update:modelValue', value: SfmaUpperExtremityPattern2TopTier): void
  (e: 'change', value: SfmaUpperExtremityPattern2TopTier): void
  (e: 'breakout-action', value: { side: UeSide; action: 'enter' | 'skip' }): void
}>()

const deepClone = (value: any) => JSON.parse(JSON.stringify(value || {}))
const model = reactive<SfmaUpperExtremityPattern2TopTier>(buildDefaultUpperExtremityPattern2TopTier())
const syncFromProps = (value?: SfmaUpperExtremityPattern2TopTier) => {
  const payload = applyUpperExtremityPattern2TopTierRules(value || buildDefaultUpperExtremityPattern2TopTier())
  model.left = payload.left
  model.right = payload.right
}
const emitModel = () => {
  const payload = applyUpperExtremityPattern2TopTierRules(deepClone(model))
  model.left = payload.left
  model.right = payload.right
...
const handleClassificationChange = (side: UeSide) => {
  ...
  const payload = applyUpperExtremityPattern2TopTierRules(next)
  model.left = payload.left
  model.right = payload.right
  emitModel()
}
...
const reset = () => {
  const payload = buildDefaultUpperExtremityPattern2TopTier()
  model.left = payload.left
  model.right = payload.right
  emitModel()
}
