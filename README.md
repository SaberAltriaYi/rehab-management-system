...
- **查询与导出**：按姓名、手机号、患者编号、治疗师、阶段、CRM 状态、性别、来源及创建
  时间筛选；导出文件包含个人信息，只能保存到受控位置。
...
报告中心将结构化评估结果整理成可复核、可追踪版本的报告。

- **生成**：从评估列表或详情生成报告，保留患者、Episode 和来源评估关系。
- **预览**：在导出前检查患者、评估编号、模块结果、风险、结论和建议。
- **复核与审批**：报告按 `draft → reviewed → approved` 流转，不同人员按权限执行复核和审批。
- **导出**：生成 DOCX 或 PDF；导出后记录报告状态和审计信息。
...
- 按患者、计划、提交角色和日期查询训练记录；
- 治疗师或文员可代录患者当日训练，记录整体完成率、训练前后疼痛、疲劳、信心和备注；
- 每个任务可标记 `completed`、`partial`、`skipped` 或 `pain_stop`，并保存实际组数、次数、
  疼痛、难度、症状和任务备注；
- “执行明细”用于核对某次打卡中每个任务的真实完成情况；
- 因疼痛停止或出现异常症状时必须如实记录，不能为了提高完成率改成“已完成”。
